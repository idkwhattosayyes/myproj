...
function openRow(rowIndex) {
  ...
  // Нашли заметку по названию — уходим в неё. Пустой query означает, что
  // искать в тексте нечего: applySearchTarget в panelSection.js не заводит
  // pendingMatch на пустую цель, поэтому заметка открывается как обычно — с
  // начала, а у заметок с openAtEnd — с прокруткой в конец, как и при входе
  // через панель. Поле ввода чистим ДО закрытия меню — иначе подписчик
