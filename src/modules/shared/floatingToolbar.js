...
// Два процента: зона сужается уже второй раз и по одной и той же причине —
// широкая хватает панель, когда её просто ведут мимо края, и поставить панель
// рядом со стенкой свободно нельзя. Было 10%, потом 5%, теперь 2% (на поле
// шириной 927px это 18px вместо 46px).
const EDGE_SNAP_RATIO = 0.02;
